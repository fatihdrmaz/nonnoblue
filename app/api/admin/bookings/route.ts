...
export async function GET() {
  ...
  const { data, error } = await admin
    .from('bookings')
    .select('id,code,status,start_date,end_date,guest_count,service_type,currency,fx_rate,total_amount,deposit_amount,balance_amount,iyzico_payment_id,contract_accepted_at,notes,created_at,boats(name),booking_extras(name,price_amount,qty)')
